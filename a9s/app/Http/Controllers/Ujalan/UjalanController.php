...
use App\Http\Resources\MySql\UjalanAcAccountResource;
  ...
  public function ac_accounts(Request $request)
  {
    ...
    if ($request->sort) {
      ...
      if (isset($sort_lists["AccountID"])) {
        ...
        if (count($first_row) > 0) {
          $model_query = $model_query->where("AccountID",$sort_symbol,$first_row["id"]);
        }
      }
      ...
      if (isset($sort_lists["AccountCode"])) {
        ...
        if (count($first_row) > 0) {
          $model_query = $model_query->where("AccountCode",$sort_symbol,$first_row["code"]);
        }
      }
      ...
      if (isset($sort_lists["AccountName"])) {
        ...
        if (count($first_row) > 0) {
          $model_query = $model_query->where("AccountName",$sort_symbol,$first_row["name"]);
        }
      }
    ...
    return response()->json([
      "data" => UjalanAcAccountResource::collection($model_query),
    ], 200);
  }
}
