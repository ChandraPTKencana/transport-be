...
namespace App\Http\Controllers\DestinationLocation;
...
use App\Models\MySql\DestinationLocation;
use App\Models\MySql\IsUser;
use App\Models\MySql\DestinationLocationTrx;
use App\Models\MySql\DestinationLocationDtl;

use App\Http\Requests\MySql\DestinationLocationRequest;

use App\Http\Resources\MySql\DestinationLocationResource;
...
class DestinationLocationController extends Controller
{
  ...
  {
    MyAdmin::checkScope($this->permissions, 'destination_location.views');
    ...
    // Init Model
    //======================================================================================================
    $model_query = new DestinationLocation();
    if (!$download) {
    ...
    return response()->json([
      "data" => DestinationLocationResource::collection($model_query),
    ], 200);
  }

  public function show(DestinationLocationRequest $request)
  {
    MyAdmin::checkScope($this->permissions, 'destination_location.view');

    // $model_query = DestinationLocation::with(['val1_by','val2_by','deleted_by','req_deleted_by'])->find($request->id);
    $model_query = DestinationLocation::find($request->id);
    return response()->json([
      "data" => new DestinationLocationResource($model_query),
    ], 200);
  }

  public function store(DestinationLocationRequest $request)
  {
    MyAdmin::checkScope($this->permissions, 'destination_location.create');
    ...
    try {
      ...
      $model_query->minimal_trip            = $request->minimal_trip;
      $model_query->bonus_trip_supir        = $request->bonus_trip_supir;
      $model_query->bonus_next_trip_supir   = $request->bonus_next_trip_supir;
      $model_query->bonus_trip_kernet       = $request->bonus_trip_kernet;
      $model_query->bonus_next_trip_kernet  = $request->bonus_next_trip_kernet;
      ...
      MyLog::sys("destination_location",$model_query->id,"insert",implode(',',$model_query->toArray()));
      ...
      if($rollback_id>-1)
      DB::statement("ALTER TABLE destination_location AUTO_INCREMENT = $rollback_id");
  ...
  public function update(DestinationLocationRequest $request)
  {
    MyAdmin::checkScope($this->permissions, 'destination_location.modify');
    
    $t_stamp        = date("Y-m-d H:i:s");
    DB::beginTransaction();
    try {
      $model_query = DestinationLocation::where("id",$request->id)->lockForUpdate()->first();
      ...
      MyLog::sys("destination_location",$request->id,"update",$SYSNOTE);
  ...
  // {
  //   MyAdmin::checkScope($this->permissions, 'destination_location.remove');
  ...
  //     $SYSNOTE = MyLib::compareChange($SYSOLD,$model_query); 
  //     MyLog::sys("destination_location",$request->id,"delete",$SYSNOTE);
  ...
  //   MyAdmin::checkMultiScope($this->permissions, ['destination_location.val','destination_location.val1','destination_location.val2']);

  //   $rules = [
  //     'id' => "required|exists:\App\Models\MySql\DestinationLocation,id",
  //   ];
  ...
  //     if(MyAdmin::checkScope($this->permissions, 'destination_location.val',true) && !$model_query->val){
  //       $model_query->val = 1;
  //       $model_query->val_user = $this->admin_id;
  //       $model_query->val_at = $t_stamp;
  //     }
  
  //     if(MyAdmin::checkScope($this->permissions, 'destination_location.val1',true) && !$model_query->val1){
  //       $model_query->val1 = 1;
  //       $model_query->val1_user = $this->admin_id;
  //       $model_query->val1_at = $t_stamp;
  //     }

  //     if(MyAdmin::checkScope($this->permissions, 'destination_location.val2',true) && !$model_query->val2){
  //       $model_query->val2 = 1;
  ...
  //     $SYSNOTE = MyLib::compareChange($SYSOLD,$model_query); 
  //     MyLog::sys("destination_location",$request->id,"approve",$SYSNOTE);
