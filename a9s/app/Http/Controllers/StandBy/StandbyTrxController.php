...
class StandbyTrxController extends Controller
{
  ...
  public function doGenPVR(Request $request){
    ...
    $id="";
    try {
      $standby_trxs = StandbyTrx::where(function($q1){$q1->where('pvr_had_detail',0)->orWhereNull("pvr_id");})->whereNull("pv_id")->where("req_deleted",0)->where("deleted",0)->where('val1',1)->
      whereIn("id",[870,871])
      ->get();
  ...
  public function genPVR($standby_trx_id){
    ...
    if($standby_trx->cost_center_code==null){
      $standby_trx->cost_center_code = '112';
      $standby_trx->cost_center_desc = 'Transport';      
  
      $get_cost_center = DB::connection('sqlsrv')->table("AC_CostCenterNames")
      ->select('CostCenter','Description')
      ->where('CostCenter','like', '112%')
      ->where('Description','like', '%'.trim($standby_trx->no_pol))
      ->first();
  
      if($get_cost_center){
        $standby_trx->cost_center_code = $get_cost_center->CostCenter;
        $standby_trx->cost_center_desc = $get_cost_center->Description;
      }
    }
