...
class SalaryBonusController extends Controller
{
  ...
  public function store(SalaryBonusRequest $request)
  {
    ...
    try {
      ...
      if($request->hasFile('attachment_1')){
        ...
        $blob = Image::make($file)->resize( null, 90, function ( $constraint ) {
  ...
  public function update(SalaryBonusRequest $request)
  {
    ...
    try {
      ...
      $location      = $model_query->photo_location;
      ...
      if ($new_image == null && $photo_preview == null) {
        if (File::exists(files_path($model_query->photo_location)) && $model_query->photo_location != null) {
          if (!unlink(files_path($model_query->photo_location)))
      ...
      if (File::exists(files_path($location)) && $location != null) {
        unlink(files_path($location));
      }
      // return response()->json([
      //   "getCode" => $e->getCode(),
...
class SalaryBonusController extends Controller
{
  ...
  public function store(SalaryBonusRequest $request)
  {
    ...
    try {
      ...
      if($request->hasFile('attachment_1')){
        $file = $request->file('attachment_1');
        $imageType = $file->getClientOriginalExtension();
        $blobFile = Image::read($file)->encode();
        $model_query->attachment_1 = $blobFile;
        $model_query->attachment_1_type = $imageType;
      }
  ...
  public function update(SalaryBonusRequest $request)
  {
    ...
    try {
      ...
      $SYSOLD      = clone($model_query);
        // $model_query->attachment_1_type = $imageType;
      $imageType     = $model_query->attachment_1_type;
